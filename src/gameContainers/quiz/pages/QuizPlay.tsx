            ...
            <div className="flex md:flex-row flex-col justify-start md:items-center items-start gap-5 mt-8">
              ...
              >
                {loading && (<svg role="status" className="inline mr-3 w-4 h-4 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span className={`relative flex items-center justify-center md:text-base text-sm font-semibold ${loading ? " text-gray-200" : " text-white"}`}>{loading ? "Processing Fee" : `Start Quiz`}{!loading && <ArrowRight className="ml-3" />}</span>
              </Button> 

              {loading ? <p className="text-xs animate-pulse text-navy italic">{firstApproval ? "Confirm Transaction in Metamask" : "One more confirmation. Please wait..."}</p> : <p className="text-xs animate-pulse text-teal italic flex items-center justify-center gap-3"><span>Transaction Completed</span> <Check /></p>}
            </div>
