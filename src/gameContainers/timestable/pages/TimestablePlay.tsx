...
export default function QuizPlay() {
  const { setScore, start, setStart, gameDetails, setGameDetails, user, allowGameProcession, setAllowGameProcession } = useContext(TimestableContext) as TimestableContextType;
  ...
  return (
    <main>
      ...
      >
        ...
        <div className="mx-auto max-w-lg text-sm shadow border border-navy p-6 rounded bg-white rounded-tl-xl rounded-br-xl relative w-full">
          {(playerTracker?.current_players === playerTracker?.total_players || playerTracker?.current_players === gameDetails?.total_players) 
          ? (
            <>
              <article className="text-gray-700">
            <h1 className="md:text-2xl text-xl font-bold mb-4 text-navy">Timestable<span className="text-tomato">{` (${gameDetails?.difficulty[0].toUpperCase()}${gameDetails?.difficulty.slice(1)})`}</span></h1>
          ...
          <div className="flex md:flex-row flex-col justify-start md:items-center items-start gap-5 mt-8">
              ...
              >
                {loading && (<svg role="status" className="inline mr-3 w-4 h-4 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span className={`relative flex items-center justify-center md:text-base text-sm font-semibold ${loading ? " text-gray-200" : " text-white"}`}>{loading ? "Processing Fee" : `Start Game`}{!loading && <FaArrowRight className="ml-3" />}</span>
              </Button> 
