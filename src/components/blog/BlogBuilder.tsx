...
const BlogBuilder = ({type, data, className, title}: BlogBuilderProps) => {
    ...
    if (type === "img") {
      ...
      >
        {data[0]}
        <a href={data[1]} target="_blank"  className="px-1 text-teal">{data[2]}</a>
        {data[3]}
      </p>
      )
    }
      ...
      return (
        ...
        <div className="w-full px-4 md:w-1/2">
          ...
          >
            {data.map((tag: string) => 
              <span key={tag} className="mr-2 mb-2 block rounded bg-teal bg-opacity-5 py-2 px-5 text-xs font-medium text-teal hover:bg-opacity-100 hover:text-white md:mr-4 lg:mr-2 xl:mr-4">
              {tag}
            </span>
      )}
          </div>
      ...
      >
        <div className="mx-auto max-w-[530px]">
          ...
          <p
            className="mb-4 text-base font-medium italic leading-[26px] text-navy"
          >
            {data[0]}
          </p>
