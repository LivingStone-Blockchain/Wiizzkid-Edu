...
export const ParagraphComponent = ({ data, className }: {data: any, className: string}) => (
    <p
      className={`${className} mb-8 md:text-base text-[15px] md:leading-8 leading-8 tracking-wide text-body-color`}
    >
      {data}
    </p>
  );
...
export const ImgComponent = ({ src, alt }:{ src: any; alt: any; }) => <img src={src} alt={alt} />;



export const ListComponent = ({ data } : {data: any}) => (
  ...
  export const QuestionnaireComponent = ({ data } : {data: any}) => (
    <ul className="wow fadeInUp md:text-base text-[15px] md:leading-8 leading-8 tracking-wide px-5 text-body-color list-disc mb-8">
      {data.map((item:any) => (
        <li key={item.question}>
          {item.question}
          <span className="font-bold block mb-4">
            Ans: <span className="font-normal pl-1">{item.answer}</span>
          </span>
        </li>
      ))}
    </ul>
  ...
  export const HeaderAndContent =  ({ data } : {data: any}) => (
    <div className="wow fadeInUp md:text-base text-[15px] md:leading-8 leading-8 tracking-wide text-body-color mb-8">
      {data.map((item:any) => (
        <p key={item.question}>
           <span className="font-bold block mt-3 mb-1">{item.header}</span>
...
export const HeaderComponent = ({ data } : {data: any}) => (
    <h2 className="wow fadeInUp mb-6 text-2xl font-bold leading-snug text-navy sm:leading-snug md:text-3xl md:leading-snug">
      {data}
    </h2>
  );


export const SubHeaderComponent = ({ data } : {data: any}) => (
    <h3 className="wow fadeInUp mb-6 text-2xl font-bold leading-snug text-navy sm:leading-snug md:leading-snug">
      {data}
    </h3>
  );
...
export const TagsShareComponent = ({ data, title } : {data: any, title:string}) => (
      ...
      <div className="w-full px-4 md:w-1/2">
        ...
        >
          {data.map((tag:any) => (
            <span
              key={tag}
              className="mr-2 mb-2 block rounded bg-teal bg-opacity-5 py-2 px-5 text-xs font-medium text-teal hover:bg-opacity-100 hover:text-white md:mr-4 lg:mr-2 xl:mr-4"
            >
              {tag}
            </span>
          ))}
...
export const QuoteComponent = ({ data } : {data: any}) => (
    <div
      className="wow fadeInUp relative z-10 mb-10 mt-2 overflow-hidden rounded bg-teal bg-opacity-5 py-8 px-6 text-center sm:p-10 md:px-[60px]"
      data-wow-delay=".1s"
    >
      <div className="mx-auto max-w-[530px]">
        <span className="mb-6 flex justify-center text-teal">
          <svg
            width="44"
            height="26"
            viewBox="0 0 44 26"
        ...
        <p className="mb-4 text-base font-medium italic leading-[26px] text-navy">
          {data[0]}
        </p>
