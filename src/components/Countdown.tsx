...
const CountDown: FC<CountdownPropTypes> = ({ day, month, year, colors }) => {
    ...
    timerComponents.push(
      <div className="col-4" key={interval}>
        <div
          ...
          className={`text-base bg-gray-200 p-3 font-semibold border-x border-gray-300`}
        >
          <p id="m-0"> {timeLeft[interval as keyof typeof timeLeft]}</p>
          <span className="text-sm leading-none font-normal text-navy">
            {" "}
            {interval}
          </span>
        </div>
  ...
  return (
    <div
      className={`bg-gray-100 border-4 text-center flex w-full lg:w-[350px] p-3 md:p-5 items-center justify-center shadow-lg rounded-md my-6`}
