...
const ForgotPassword = () => {
  const { forgotPasswordEmail, setForgotPasswordEmail, handleForgotPassword, isLoading } = useContext(UserContext) as UserContextType;
  
  return (
    <Form alt='password update' img={password}>
      <form onSubmit={handleForgotPassword} className="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
        <div className="w-full flex flex-col md:gap-6 gap-4">
          ...
          <button type="submit" className={`my-1 relative flex h-11 w-full items-center justify-center px-6 before:absolute before:inset-0 rounded-full ${isLoading ? "cursor-not-allowed bg-[#37385e]" : "cursor-pointer transition duration-300 hover:scale-105 active:duration-75 active:scale-95 bg-[#252641]"}`}>
            ...
            <span className={`relative md:text-base text-s font-semibold ${isLoading ? " text-gray-200" : " text-white"}`}>{isLoading ? "Processing" : "Recover Password"}</span>
          </button>
        </div>
      </form>
    </Form>
